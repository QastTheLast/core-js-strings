...
function removeFirstOccurrences(str, value) {
  const foundIndex = str.indexOf(value);
  if (foundIndex !== -1) {
    const newStr =
      str.substring(0, foundIndex) + str.substring(foundIndex + value.length);
    return newStr;
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  const properMinutes = String(minutes).padStart(2, '0');
  const properSeconds = String(seconds).padStart(2, '0');
  const formattedTime = `${properMinutes}:${properSeconds}`;
...
function countVowels(str) {
  const vowelsFound = str.match(/[aeiouy]/gi);
  if (vowelsFound) {
...
function isPalindrome(str) {
  const cleanString = str
    ...
    .toLowerCase()
    .trim();
  return Boolean(cleanString.split('').reverse().join('') === cleanString);
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (letter) => alphabetPlus[alphabetPlus.indexOf(letter) + 13]
  );
}
